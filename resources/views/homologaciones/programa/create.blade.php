...
@section('title', 'SaiQ - Homologacion por Programa')
...
@php
    ...
    $selectedExt = old('ext', $draft->programa_ext_cod ?? '');
    ...
    $programasProcedenciaAgrupados = collect($programasProcedencia ?? [])->groupBy('programa_pca_cod');
    $selectedExtRendered = false;
@endphp
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                ...
                <h3 style="font-size: 18px; font-weight: 600; margin-bottom: 35px; color: #555;">Homologaci&oacute;n por Programa</h3>
                ...
                @if($draft)
                    <div class="alert alert-info">Editando homologaci&oacute;n No. {{ $draft->id }} en estado {{ strtoupper($draft->estado ?? 'borrador') }}. Al guardar o generar se actualizar&aacute; este mismo consecutivo.</div>
                @endif

                <form action="{{ route('homologaciones.programa.generate') }}" method="post" name="fff" style="max-width: 1000px;">
                    @csrf
                    <input type="hidden" name="_accion" value="generar">
                    @if($draft)
                    ...
                    <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; border: 1px solid #ddd; margin-bottom: 20px;">
                        ...
                        <div class="form-group" style="margin-bottom: 0;">
                            ...
                            <select name="ext" id="ext" class="form-control" style="padding: 10px 12px; font-size: 14px; border: 1px solid #d0d0d0; border-radius: 4px; background-color: white;">
                                <option value="0">[Seleccione]</option>
                                @if($programasProcedenciaAgrupados->isNotEmpty())
                                    @foreach($programasProcedenciaAgrupados as $programaPcaCod => $programasGrupo)
                                        <optgroup label="{{ $programasGrupo->first()->programa_pca_nombre }}">
                                            @foreach($programasGrupo as $programa)
                                                @php
                                                    $isSelectedExt = !$selectedExtRendered
                                                        && $selectedExt === $programa->cod
                                                        && ($selectedPca === '' || $selectedPca === $programa->programa_pca_cod);
                                                    if ($isSelectedExt) {
                                                        $selectedExtRendered = true;
                                                    }
                                                @endphp
                                                <option value="{{ $programa->cod }}" data-pca="{{ $programa->programa_pca_cod }}" @selected($isSelectedExt)>{{ $programa->nombre }} - {{ $programa->institucion_nombre }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                @else
                                    @foreach($programasExternos as $programa)
                                        <option value="{{ $programa->cod }}" @selected($selectedExt === $programa->cod)>{{ $programa->nombre }} - {{ $programa->institucion_nombre }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div style="text-align: center; margin-top: 20px;">
                        ...
                        <button type="button" class="btn btn-primary" onclick="guardar();" style="padding: 12px 30px; font-size: 14px; font-weight: 600; background-color: #0066cc; color: white; border: none; border-radius: 4px; cursor: pointer; margin-right: 10px;">Generar PDF</button>
...
function guardar() {
    ...
    document.fff._accion.value = 'generar';
    document.fff.target = '_blank';
    document.fff.submit();
}
...
(function() {
    ...
    var relacionesProcedencia = @json(($relacionesProcedencia ?? collect())->map(fn ($relacion) => ['pca' => $relacion->programa_pca_cod, 'ext' => $relacion->programa_ext_cod])->values());
    ...
    function filtrarProgramasProcedencia() {
        if (!programaPca || !programaExt) {
            return;
        }

        var relacionados = relacionesProcedencia
            .filter(function(relacion) { return relacion.pca === programaPca.value; })
            .map(function(relacion) { return relacion.ext; });

        Array.prototype.forEach.call(programaExt.options, function(option) {
            var optionPca = option.getAttribute('data-pca');
            var visible = option.value === '0'
                || programaPca.value === '0'
                || (optionPca ? optionPca === programaPca.value : relacionados.length === 0 || relacionados.indexOf(option.value) !== -1);

            option.hidden = !visible;
            option.disabled = !visible;
        });

        Array.prototype.forEach.call(programaExt.querySelectorAll('optgroup'), function(group) {
        ...
        if (programaExt.selectedOptions.length && programaExt.selectedOptions[0].disabled) {
            programaExt.value = '0';
        }
    }
